  ...
  return (
    <main className="mx-auto min-h-screen max-w-6xl space-y-6 px-6 py-10">
      ...
      <Tabs defaultValue={defaultTab}>
        ...
        <TabsContent value="packs"><div className="grid gap-4">{instance.packs.length ? instance.packs.map((pack) => <Card key={pack.id}><CardHeader><CardTitle className="flex items-center gap-2 text-base"><FileArchive className="size-4" />{pack.originalName}</CardTitle><CardDescription>{pack.active ? "Active revision" : "Archived revision"} · {Math.round(pack.sizeBytes / 1024)} KiB</CardDescription></CardHeader><CardContent className="space-y-3">{pack.sha256 && <p className="break-all font-mono text-xs text-muted-foreground">SHA-256 {pack.sha256}</p>}{pack.downloadable ? <Button asChild variant="outline" size="sm"><a href={`/api/instances/${id}/packs/${pack.id}/download`} download><Download className="size-4" />Download pack</a></Button> : <Button variant="outline" size="sm" disabled><Download className="size-4" />File unavailable</Button>}</CardContent></Card>) : <Placeholder title="No pack revisions" description="Generated-world instances do not have a source pack." />}</div></TabsContent>
